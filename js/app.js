...
class EnglishHeroApp {
    ...
    navigate(page) {
        ...
        const pageMap = { home: 0, words: 1, sentences: 2, reading: 3, games: 4, profile: 5 };
    ...
    renderReading() {
        ...
        // 重置显示
        const quizContent = document.getElementById('reading-quiz-content');
    ...
    selectReadingUnit(unitId) {
        ...
        if (!unitData || !unitData.articles || unitData.articles.length === 0) {
            alert('该单元暂无题目');
            return;
        }
        
        // 获取该单元所有题目
        this.currentUnitQuestions = [];
        unitData.articles.forEach(article => {
            if (article.questions) {
                article.questions.forEach((q, idx) => {
                    this.currentUnitQuestions.push({
                        ...q,
        ...
        if (this.currentUnitQuestions.length === 0) {
            alert('该单元暂无题目');
            return;
        }
    ...
    renderQuizQuestion() {
        ...
        if (!this.currentUnitQuestions || this.currentUnitQuestions.length === 0) return;
        
        const question = this.currentUnitQuestions[this.currentQuestionIndex];
        
        if (questionText) {
            questionText.innerHTML = `<span style="color: var(--primary); font-size: 14px;">[${question.articleTitle}]</span><br>
                ${this.currentQuestionIndex + 1}. ${question.question}`;
        }
        
        if (quizOptions) {
            quizOptions.innerHTML = question.options.map((option, i) => `
                <button class="question-option" onclick="app.answerQuizQuestion(${i})" 
                    style="padding: 15px; border: 2px solid #e0e0e0; border-radius: 10px; background: white; 
                           cursor: pointer; text-align: left; font-size: 16px; transition: all 0.2s;"
        ...
        if (quizProgress) {
            quizProgress.textContent = `第 ${this.currentQuestionIndex + 1} / ${this.currentUnitQuestions.length} 题`;
        }
    }
    ...
    answerQuizQuestion(answerIndex) {
        ...
        // 显示正确/错误
        const options = document.querySelectorAll('#quiz-options .question-option');
        if (options[answerIndex]) {
            options[answerIndex].style.borderColor = correct ? 'var(--success)' : 'var(--danger)';
            options[answerIndex].style.background = correct ? '#E8F5E9' : '#FFEBEE';
        }
        if (options[question.answer]) {
            options[question.answer].style.borderColor = 'var(--success)';
            options[question.answer].style.background = '#E8F5E9';
        }
        ...
        setTimeout(() => {
            ...
            if (this.currentQuestionIndex < this.currentUnitQuestions.length) {
                this.renderQuizQuestion();
            } else {
                this.showQuizResult();
            }
    ...
    showQuizResult() {
        ...
        const totalQuestions = this.currentUnitQuestions.length;
        const accuracy = Math.round((correctCount / totalQuestions) * 100);
        
        // 计算XP
        let xpEarned = Math.round(100 * (accuracy / 100));
        if (accuracy === 100) xpEarned += 20;
        
        this.addXP(xpEarned);
        
        // 标记完成
        if (!this.state.completedReadings.includes(this.currentUnitId)) {
            this.state.completedReadings.push(this.currentUnitId);
        ...
        if (quizContent) {
            quizContent.innerHTML = `
                <div style="text-align: center; padding: 30px 20px;">
                    ...
                    <h2>🎯 答题完成！</h2>
                    <p style="font-size: 24px; font-weight: 700; margin: 16px 0;">
                        ${correctCount}/${totalQuestions} 正确 (${accuracy}%)
                    </p>
                    <p style="color: var(--text-secondary); margin-bottom: 24px;">
                        获得 ${xpEarned} XP
                    </p>
                    <div style="background: white; padding: 20px; border-radius: 12px; margin: 20px 0; text-align: left;">
                        ${this.readingAnswers.map((a, i) => `
                            <div style="padding: 10px 0; border-bottom: 1px solid #eee; ${a.correct ? '' : 'color: var(--danger);'}">
                                ${i + 1}. ${a.correct ? '✅' : '❌'} ${a.question.substring(0, 30)}...
                            </div>
                        `).join('')}
                    </div>
                    <button class="btn-primary" onclick="app.navigate('reading')" style="margin-top: 20px;">继续做题</button>
                </div>
            `;
        }
    }
