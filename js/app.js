...
class EnglishHeroApp {
    ...
    selectPartType(type) {
        ...
        if (type === correct) {
            ...
            // 显示正确反馈
            if (feedbackEl) {
                feedbackEl.innerHTML = `<div style="color: #4CAF50; font-weight: bold; padding: 10px; background: #E8F5E9; border-radius: 8px; margin-top: 10px;">
                    ✅ 回答正确！+10 XP
                </div>`;
            }
        } else {
            ...
            // 显示错误反馈和正确答案
            if (feedbackEl) {
                feedbackEl.innerHTML = `<div style="color: #f44336; font-weight: bold; padding: 10px; background: #FFEBEE; border-radius: 8px; margin-top: 10px;">
                    ❌ 回答错误！正确答案是：${correct}
                </div>`;
            }
